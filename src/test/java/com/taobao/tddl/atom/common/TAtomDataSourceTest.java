...
public class TAtomDataSourceTest extends BaseAtomTest {
	static class a {
		...
		void p() throws Throwable {
			...
			List<Integer> queryForList = jtp.queryForList("select * from aa", Integer.class);
	...
	private void testChangePasswd(ChangeTestConfig change, ChangeTestConfig restore, String dbType)
			throws IOException, AtomAlreadyInitException, Exception {
		String appName = "tddl_sample";
		...
		MockServer.setConfigInfo(passwdDataId, PropLoadTestUtil.convertProp2Str(
				restore.doChange(PropLoadTestUtil.loadPropFromFile("conf/" + configName + "/passwd.properties"))));
		Thread.sleep(3000);
		// 期待结果正常
		actual = jtp.queryForObject(testSql, Integer.class);
		Assert.assertEquals(actual, 1);
	...
	private void testChange(ChangeTestConfig change, ChangeTestConfig restore, String type, String dbType,
		...
		Assert.assertTrue(result);
		MockServer.setConfigInfo(dataId, PropLoadTestUtil
				.convertProp2Str(restore.doChange(PropLoadTestUtil.loadPropFromFile("conf/" + configName + fileName))));
		Thread.sleep(3000);
		// 期待结果正常
		actual = jtp.queryForObject(testSql, Integer.class);
		Assert.assertEquals(actual, 1);
	...
	private static TAtomDataSource createTAtomDataSource(String appName, String dbKey, String configName)
		...
		MockServer.setConfigInfo(TAtomConstants.getPasswdDataId(tAtomDsConfDO.getDbName(), tAtomDsConfDO.getDbType(),
